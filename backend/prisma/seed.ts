...
async function main() {
  console.log('  ⚠️  Cleaning up all existing records in the database...');
  ...
  // 5. Create Default AI Provider Config (Gemini)
  ...
  console.log('  ✓ AI Provider config created (Gemini)');
 
  // 6. Create Default Assistant
  const assistant = await prisma.assistant.create({
    data: {
      name: 'Realtime Voice Assistant',
      systemInstruction: 'You are CallAI, a real-time voice assistant. At the very start of the call, you must greet the user with exactly this sentence: "Hi, this is CallAI. You can ask me anything." Say only this sentence for the first message, do not add anything else, do not continue speaking, and wait for the user to respond. After the greeting, keep responses short and conversational, speak like a human, not like a robot, and avoid long explanations unless asked.',
      voiceId: 'alloy',
