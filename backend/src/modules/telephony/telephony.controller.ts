...
export class TelephonyController {
  ...
  async handleExotelInboundCall(@Body() body: InboundCallDto) {
    ...
    return {
      ...
      instruction: {
        action: 'stream',
        url: this.buildStreamUrl(voiceGatewayUrl, '/v1/voice-stream', session.id),
  ...
  async handleTwilioInboundCall(@Body() body: any) {
    ...
    const voiceGatewayUrl = process.env.VOICE_GATEWAY_WS_URL || 'ws://localhost:3000/v1/voice-stream';
    const twilioGatewayUrl = this.buildStreamUrl(
      voiceGatewayUrl,
      '/v1/telephony/twilio/stream',
      session.id,
    );
...
<Response>
  <Connect>
    <Stream url="${this.escapeXmlAttribute(twilioGatewayUrl)}">
      <Parameter name="callSessionId" value="${this.escapeXmlAttribute(session.id)}" />
    </Stream>
  </Connect>
</Response>`;
  }
  ...
  private buildStreamUrl(baseUrl: string, path: string, callSessionId: string): string {
    const streamUrl = new URL(baseUrl);
    streamUrl.pathname = path;
    streamUrl.searchParams.set('callSessionId', callSessionId);
    streamUrl.searchParams.set('token', process.env.VOICE_GATEWAY_TOKEN || 'bypass-auth');
